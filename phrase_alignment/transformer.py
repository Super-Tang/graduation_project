...
from phrase_alignment.desc import *
...
def multi_head_attention(queries,
    ...
    ctx_multiheads, weights = scaled_dot_product_attention(q, k, v, attn_bias, d_model,
                                                  dropout_rate)
    

    out = __combine_heads(ctx_multiheads)
    # weights = __combine_weights(weights)
...
def decoder_layer(dec_input,
    ...
    """
    slf_attn_output, _ = multi_head_attention(
    ...
    slf_attn_output = post_process_layer(
        ...
        postprocess_cmd,
        prepostprocess_dropout, )
    enc_attn_output, weight_matrix = multi_head_attention(
    ...
    dec_output = post_process_layer(
        ...
        postprocess_cmd,
        prepostprocess_dropout, )
    return dec_output, weight_matrix
...
def decoder(dec_input,
    ...
    """
    for i in range(n_layer):
        dec_output, weight_matrix = decoder_layer(
            dec_input,
            enc_output,
            dec_slf_attn_bias,
            dec_enc_attn_bias,
...
def transformer(src_vocab_size,
    ...
    predict, weight_matrix = wrap_decoder(
        trg_vocab_size,
        max_length,
        n_layer,
        n_head,
    ...
    return sum_cost, avg_cost, predict, token_num, weight_matrix, reader if use_py_reader else None
...
def wrap_decoder(trg_vocab_size,
    ...
    dec_output, weight_matrix = decoder(
        dec_input,
        enc_output,
        trg_slf_attn_bias,
        trg_src_attn_bias,
...
def fast_decode(src_vocab_size,
    ...
    return weight_matrix, reader if use_py_reader else None
    '''
    def beam_search():
        ...
        with while_op.block():
            ...
            pre_pos = layers.elementwise_mul(
                ...
                y=step_idx,
                axis=0)
            logits, weight_matrix = wrap_decoder(
                trg_vocab_size,
                max_in_len,
                n_layer,
                n_head,
        ...
        return finished_ids, finished_scores, weight_matrix

    finished_ids, finished_scores, weight_matrix = beam_search()
    return finished_ids, finished_scores, weight_matrix, reader if use_py_reader else None
    '''


def create_net(is_training, model_input, args):
    if is_training:
        sum_cost, avg_cost, _, token_num, weight_matrix, _ = transformer(
            args.src_vocab_size,
            args.trg_vocab_size,
            args.max_length + 1,
            args.n_layer,
        ...
        return sum_cost, avg_cost, token_num, weight_matrix
    else:
        weight_matrix, _ = fast_decode(
            args.src_vocab_size,
            args.trg_vocab_size,
            args.max_length + 1,
            args.n_layer,
