...
def do_predict(args):
    ...
    with fluid.program_guard(test_prog, startup_prog):
        with fluid.unique_name.guard():
            ...
            weight_matrix = create_net(
                is_training=False, model_input=input_field, args=args)

            weight_matrix.persistable = True 
    ...
    try:
            ...
            weight_matrix = exe.run(
                test_prog,
                fetch_list=[weight_matrix],return_numpy=False)
            print('hreerere')
            print(np.array(weight_matrix[0])[0][2])
