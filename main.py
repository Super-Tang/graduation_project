...
from ui.open import File_Window
...
class Main_UI(object):
    ...
    def setupUi(self, QMainWindow, arg):
        ...
        self.timer1 = QTimer()
        self.timer1.timeout.connect(self.open_source)
        self.timer2 = QTimer()
        self.timer2.timeout.connect(self.open_target)
    ...
    def open_source(self):
        ...
        if not self.file_window.finished:
            self.timer1.start(100)
        else:
            self.timer1.stop()
            self.file_window.close()
            self.textedit.setText(self.file_window.file_name)
            self.source_text = self.file_window.text
            self.file_window = None
    ...
    def open_target(self):
        ...
        if not self.file_window.finished:
            ...
            self.timer2.stop()
            self.file_window.close()
            self.textedit1.setText(self.file_window.file_name)
            self.target_text = self.file_window.text
            self.file_window = None
    ...
    def fill_table(self, is_source, is_list=False):
        ...
        if is_list:
            ...
            if count <= self.max_table_row:
                if self.table.rowCount() != (count+1):
                    self.table.setRowCount(count+1)
                if is_source:
                    for i in range(count):
                        if i + self.current_source < self.source_count:
        ...
        if source_language >= 0:
            ...
            if len(self.source_text) > 0 and len(self.target_text) > 0:
                ...
                if source_language == Globals.ENGLISH:
                    self.args.language = Globals.ENGLISH
                    # self.args.predict_file = self.args.predict_file + 'en_test.bpe'
                    ...
                    self.args.language = Globals.FRENCH
                    # self.args.predict_file = self.args.predict_file + 'fr_test.bpe'
                    ...
                    self.args.language = Globals.GERMAN
                    # self.args.predict_file = self.args.predict_file + 'de_test.bpe'
                    ...
                    self.args.language = Globals.RUSSIAN
                    # self.args.predict_file = self.args.predict_file + 'ru_test.bpe'
                    ...
                    self.args.language = Globals.JAPAN
                    # self.args.predict_file = self.args.predict_file + 'jp_test.bpe'
                    self.args.init_from_params = self.args.jp_init_from_params
                    self.args.src_vocab_fpath = self.args.jp_src_vocab_fpath
                    self.args.trg_vocab_fpath = self.args.jp_trg_vocab_fpath
                    self.args.bpe_model_dir = self.args.jp_bpe_model_dir
                    save_content = Chinese_Japan.pip_line(source_text, target_text, False)
                elif source_language == Globals.ARABIA:
                    self.args.language = Globals.ARABIA
                    # self.args.predict_file = self.args.predict_file + 'ar_test.bpe'
                    self.args.init_from_params = self.args.ar_init_from_params
                    self.args.src_vocab_fpath = self.args.ar_src_vocab_fpath
                    self.args.trg_vocab_fpath = self.args.ar_trg_vocab_fpath
                    self.args.bpe_model_dir = self.args.ar_bpe_model_dir
                    save_content = Chinese_Arabia.pip_line(source_text, target_text, False)
                elif source_language == Globals.KOREAN:
                    self.args.language = Globals.KOREAN
                    # self.args.predict_file = self.args.predict_file + 'ko_test.bpe'
                    self.args.init_from_params = self.args.ko_init_from_params
                    self.args.src_vocab_fpath = self.args.ko_src_vocab_fpath
                    self.args.trg_vocab_fpath = self.args.ko_trg_vocab_fpath
                    self.args.bpe_model_dir = self.args.ko_bpe_model_dir
                    save_content = Chinese_Korean.pip_line(source_text, target_text, False)
                elif source_language == Globals.SPAIN:
                    self.args.language = Globals.SPAIN
                    # self.args.predict_file = self.args.predict_file + 'es_test.bpe'
                    ...
                    self.args.language = Globals.PORTUGAL
                    # self.args.predict_file = self.args.predict_file + 'pt_test.bpe'
                    self.args.init_from_params = self.args.pt_init_from_params
                    self.args.src_vocab_fpath = self.args.pt_src_vocab_fpath
                    self.args.trg_vocab_fpath = self.args.pt_trg_vocab_fpath
                    self.args.bpe_model_dir = self.args.pt_bpe_model_dir
            ...
            self.errors.show()
        if self.phrase_alignment is None or self.prev_language is None or self.prev_language != source_language:
            self.prev_language = source_language
...
if __name__ == "__main__":
    args = PDConfig(yaml_file="cof.yaml")
    args.build()
    args = default(args)
